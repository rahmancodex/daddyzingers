...
import { uploadMenuImage, validateImageFile } from "@/lib/admin-menu-storage";

type Props = {
  value: string | null;
  onChange: (url: string | null) => void;
  prefix?: "items" | "categories";
  aspect?: "square" | "wide";
  label?: string;
};

export function ImageUploader({ value, onChange, prefix = "items", aspect = "wide", label = "Image" }: Props) {
  ...
  async function handleFile(file: File) {
    ...
    setPct(0);
    try {
      const { url } = await uploadMenuImage(file, { prefix, onProgress: setPct });
      onChange(url);
